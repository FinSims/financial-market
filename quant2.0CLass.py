...
class StockPredictor:
    ...
    def generate_trading_signals(self, test_results_df, neutral_threshold=0.01):
    ...
    def predict_stock(self, ticker):
        ...
        model = self.LSTM(1, self.hidden_size,
        ...
        dummies = np.zeros((X_test.shape[0], self.lookback + 1))
        dummies[:, 0] = test_predictions
        test_predictions = scaler.inverse_transform(dummies)[:, 0]
        dummies[:, 0] = y_test.flatten()
        new_y_test = scaler.inverse_transform(dummies)[:, 0]
